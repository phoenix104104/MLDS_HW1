...
class DNN:
    def __init__(self, structure, learning_rate=0.05, epoch=10, batch_size=100, dropout_prob=0.5):
        ...
        self.train_model = T.dot(hidden, self.W[layer_num-1]) + self.B[layer_num-1]
        ...
        for i in range(layer_num-1):
            hidden = sigmoid(T.dot(hidden, self.W[i]) + self.B[i])
            hidden *= (1 - self.dropout_prob) 

        self.test_model = T.dot(hidden, self.W[layer_num-1]) + self.B[layer_num-1]


        # TODO: cost
        self.cost = cost_func(self.train_model, self.Y)

        # TODO: updates(sgd)
        self.updates = sgd(self.cost, self.W+self.B, self.lr)

        # TODO: y_pred
        self.y_pred = T.argmax(self.train_model, axis=1)
